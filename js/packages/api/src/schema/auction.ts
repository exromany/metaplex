...
import { getSafetyDepositBoxesExpected } from '@oyster/common/dist/lib/models/metaplex/index';
...
export const BidState = objectType({
  ...
  definition(t) {
    ...
    t.nonNull.list.field('bids', { type: nonNull(Bid) });
    t.nonNull.bn('max');
  },
});
...
export const Auction = objectType({
  ...
  definition(t) {
    ...
    t.nonNull.field('bids', {
      ...
      resolve: (item, args, { api }) =>
        api.state.then(state => listWrapPubkey(getAuctionBids(item, state))),
    });
  ...
  definition(t) {
    ...
    t.nonNull.bn('safetyDepositBoxesExpected', {
      resolve: async (item, args, { api }) => {
        ...
        const vault = getVault(item, state);
        return getSafetyDepositBoxesExpected(item, vault);
      },
    });
    ...
    t.nullable.field('participationConfig', {
      ...
      resolve: (item, args, { api }) =>
        api.state.then(state => getParticipationConfig(item, state)),
    });
  },
});
